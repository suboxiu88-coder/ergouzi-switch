...
/// - `"codex_oauth"` + "Please re-login via ergouzi-switch."（Ergouzi Switch 自管 OAuth 路径）
...
/// Gemini OAuth Client 凭据不提交到公开仓库。
/// 如需启用 Gemini 官方额度查询，请在运行环境设置这两个变量。
fn gemini_oauth_client_credentials() -> Option<(String, String)> {
    let client_id = std::env::var("ERGOUZI_GEMINI_OAUTH_CLIENT_ID").ok()?;
    let client_secret = std::env::var("ERGOUZI_GEMINI_OAUTH_CLIENT_SECRET").ok()?;
