...
pub const PROFILE_NAME: &str = "Ergouzi Switch";
...
mod tests {
    ...
    #[test]
    fn claude_desktop_official_id_with_base_url_is_direct_provider() {
        let mut provider = direct_provider(CLAUDE_DESKTOP_OFFICIAL_PROVIDER_ID);
        provider.name = "Claude Desktop Official".to_string();
        provider.category = Some("official".to_string());

        assert!(!is_official_provider(&provider));
        assert!(is_compatible_direct_provider(&provider));
    }
