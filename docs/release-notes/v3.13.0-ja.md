...
- **軽量モード**: トレイ専用の動作モード。トレイへの終了時にメインウィンドウを破棄し、必要時に再作成することで、アイドル時の Ergouzi Switch のデスクトップフットプリントを最小化
...
- **Skills 発見とバッチ更新**: SHA-256 ベースの skill 更新検出、各 skill および「すべて更新」のバッチ更新、`skills.sh` 検索統合、Ergouzi Switch ストレージと `~/.agents/skills` の間のストレージ位置切り替え
...
Ergouzi Switch のアイドル時のデスクトップフットプリントを大幅に削減するトレイ専用動作モード。

- トレイへの終了時にメインウィンドウを隠すのではなく破棄し、UI リソースとメモリを解放
- トレイ、ディープリンク、またはシングルインスタンスアクティベーションからユーザーが Ergouzi Switch を再オープンしたときにウィンドウを再作成
...
- **ストレージ位置切り替え**: Ergouzi Switch ストレージと `~/.agents/skills` の間を skill 状態を失わずに切り替え
...
新規インストールのユーザーに Ergouzi Switch のワークフローを案内するワンタイムウェルカムダイアログを追加。
...
これらの機能を有効にすることで、ユーザーは**すべてのリスクを自己責任で負う**ものとします。Ergouzi Switch は、これらの機能の使用に起因するアカウント制限、警告、またはサービス停止について一切の責任を負いません。

---

## ダウンロードとインストール

[Releases](https://github.com/suboxiu88-coder/ergouzi-switch/releases/latest) から適切なバージョンをダウンロードしてください。
...
| ファイル                                   | 説明                                                 |
| ------------------------------------------ | ---------------------------------------------------- |
| `Ergouzi-Switch-v3.13.0-Windows.msi`            | **推奨** - MSI インストーラー、自動更新対応          |
| `Ergouzi-Switch-v3.13.0-Windows-Portable.zip`   | ポータブル版、解凍して実行、レジストリ書き込みなし   |

### macOS

| ファイル                           | 説明                                                              |
| ---------------------------------- | ----------------------------------------------------------------- |
| `Ergouzi-Switch-v3.13.0-macOS.dmg`     | **推奨** - DMG インストーラー、ドラッグ＆ドロップでインストール   |
| `Ergouzi-Switch-v3.13.0-macOS.zip`     | 解凍して Applications にドラッグ、Universal Binary                |
| `Ergouzi-Switch-v3.13.0-macOS.tar.gz`  | Homebrew インストールと自動更新用                                 |
