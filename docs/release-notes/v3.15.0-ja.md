...
> ## 唯一の公式チャネル（必ずお読みください）
>
> Ergouzi Switch は**完全に無料・オープンソース**のデスクトップアプリで、**ユーザーから料金を徴収することはありません**。最近、Ergouzi Switch の名を騙って課金を要求したり認証情報を収集する偽サイトが複数確認されており、一部のユーザーには既に金銭的被害が発生しています。本ソフトウェアは下記の公式チャネルからのみ入手してください：
>
> | チャネル     | 唯一の公式                                                                     |
> | ------------ | ------------------------------------------------------------------------------ |
> | 公式サイト   | **[ergouzi.life](https://ergouzi.life)**                                         |
> | ソースコード | **[github.com/suboxiu88-coder/ergouzi-switch](https://github.com/suboxiu88-coder/ergouzi-switch)** |
> | ダウンロード | **[GitHub Releases](https://github.com/suboxiu88-coder/ergouzi-switch/releases)**        |
> | 作者         | **[@farion1231](https://github.com/farion1231)**                               |
> | 偽サイト通報 | **[GitHub Issues](https://github.com/suboxiu88-coder/ergouzi-switch/issues)**            |
>
> **料金請求・チャージ・認証情報の提供を求める「Ergouzi Switch」サイトやクライアントはすべて偽物です。** 支払いを誘導された場合は、直ちに取引を中止し、偽サイトを速やかに削除できるよう GitHub Issues からご報告ください。
...
**[Ergouzi Switch で Claude Desktop プロバイダーを一括設定・管理・切り替える](../user-manual/ja/2-providers/2.6-claude-desktop.md)**
...
Ergouzi Switch は初めて **Claude Desktop** を一等管理対象パネルとして扱い、Claude Code / Codex / Gemini / OpenCode / OpenClaw / Hermes と並列に位置づけます。

- Claude Desktop 専用パネルを追加し、Ergouzi Switch 内蔵プロキシゲートウェイを介してサードパーティプロバイダーを Claude Desktop に代理転送
...
ChatGPT Codex 系プロバイダーはハードコードされたモデル選択に依存しなくなり、Ergouzi Switch が必要に応じて ChatGPT バックエンドから**ライブモデルリスト**を取得します。
...
Claude Desktop のプロバイダーを切り替えると Ergouzi Switch 管理の 3P プロファイルが書き込まれます。**Claude Desktop の再起動**が必要です; プロキシモードのプロバイダーは、使用中 Ergouzi Switch の Local Routing が動作し続けている必要があります。
...
### Ergouzi Switch ブランド公式サイトを ergouzi.life に統一

アプリ内および README 内のすべての「公式サイト」参照を、唯一の公式サイトとして ergouzi.life に統一しました; Release notes テンプレートにも ergouzi.life を反映。
...
Release notes テンプレート内に `ergouzi.life` を反映しました。

### ブランド公式サイト

各 README およびアプリ内参照で `ergouzi.life` を唯一の公式サイトとしてドキュメント化しました。
...
これら 20 個のプリセットは以前はデフォルトでプロキシ経由でルーティングされていましたが、現在はデフォルトで直接接続です。アップグレード前にこのうちのいずれかを使用しており、かつプロキシルーティングの接続性に依存していた場合（例: プロキシに特殊な補正器や変換層がある場合）、接続性を検証してください; 必要に応じて、Ergouzi Switch パネル内で手動でプロキシモードに戻すことができます。

### Claude Desktop の操作制約

Claude Desktop プロバイダーの切り替えには、**Claude Desktop の再起動**が必要です; プロキシモードのプロバイダーは、使用中 Ergouzi Switch の Local Routing が動作し続けている必要があります — Ergouzi Switch を終了させたり Local Routing を停止させたりすると、プロキシモードの Claude Desktop プロバイダーへの接続が切断されます。
...
**Claude Desktop サードパーティプロバイダーのプロキシ切り替え**: Ergouzi Switch 内蔵プロキシゲートウェイ経由で Claude Desktop のリクエストをサードパーティプロバイダーに転送する際、サードパーティプロバイダーの課金、コンプライアンス、データ保持に関する制約はそれぞれ異なります。利用前にターゲットプロバイダーの利用規約をお読みください。

ユーザーが上記機能を有効化することで、**すべてのリスクを自己責任で**受諾したものとみなされます。Ergouzi Switch は、これらの機能の使用に起因するアカウントの制限、警告、サービス停止について一切の責任を負いません。

---

## ダウンロード・インストール

[Releases](https://github.com/suboxiu88-coder/ergouzi-switch/releases/latest) から対応バージョンをダウンロードしてください。
...
| ファイル                                 | 説明                                        |
| ---------------------------------------- | ------------------------------------------- |
| `Ergouzi-Switch-v3.15.0-Windows.msi`          | **推奨** - MSI インストーラー、自動更新対応 |
| `Ergouzi-Switch-v3.15.0-Windows-Portable.zip` | ポータブル版、解凍して実行、レジストリ不要  |

### macOS

| ファイル                         | 説明                                                   |
| -------------------------------- | ------------------------------------------------------ |
| `Ergouzi-Switch-v3.15.0-macOS.dmg`    | **推奨** - DMG インストーラー、Applications にドラッグ |
| `Ergouzi-Switch-v3.15.0-macOS.zip`    | 解凍して Applications にドラッグ、Universal Binary     |
| `Ergouzi-Switch-v3.15.0-macOS.tar.gz` | Homebrew インストールと自動更新用                      |
...
| --------------------------------------- | ----------- | -------------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i Ergouzi-Switch-*.deb` または `sudo apt install ./Ergouzi-Switch-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i Ergouzi-Switch-*.rpm` または `sudo dnf install ./Ergouzi-Switch-*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./Ergouzi-Switch-*.rpm`                                    |
| Arch Linux / Manjaro                    | `.AppImage` | 実行権限を付与して実行、または AUR を使用                                  |
| その他のディストリビューション / 不明   | `.AppImage` | `chmod +x Ergouzi-Switch-*.AppImage && ./Ergouzi-Switch-*.AppImage`                  |
