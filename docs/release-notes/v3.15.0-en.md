...
>
> Ergouzi Switch is a **fully free and open-source** desktop app, and we **do not charge users any fees**. Multiple imposter websites have recently been spotted impersonating Ergouzi Switch to solicit payments and harvest account credentials, with some users already reporting financial losses. Please only obtain the software through the official channels listed below:
>
> | Channel            | Only Official                                                                  |
> | ------------------ | ------------------------------------------------------------------------------ |
> | Website            | **[ergouzi.life](https://ergouzi.life)**                                         |
> | Source             | **[github.com/suboxiu88-coder/ergouzi-switch](https://github.com/suboxiu88-coder/ergouzi-switch)** |
> | Downloads          | **[GitHub Releases](https://github.com/suboxiu88-coder/ergouzi-switch/releases)**        |
> | Author             | **[@farion1231](https://github.com/farion1231)**                               |
> | Report an Imposter | **[GitHub Issues](https://github.com/suboxiu88-coder/ergouzi-switch/issues)**            |
>
> **Any "Ergouzi Switch" website or client that asks you for payment, top-ups, or login credentials is fake.** If you have been tricked into paying, stop the transaction immediately and file a report through GitHub Issues so we can take down the imposter site as quickly as possible.
...
**[Use Ergouzi Switch to configure, manage, and switch Claude Desktop providers in one place](../user-manual/en/2-providers/2.6-claude-desktop.md)**
...
Ergouzi Switch now treats **Claude Desktop** as a first-class managed surface alongside Claude Code / Codex / Gemini / OpenCode / OpenClaw / Hermes.

- New dedicated Claude Desktop panel that brokers third-party providers to Claude Desktop through Ergouzi Switch's in-app proxy gateway
...
ChatGPT Codex providers no longer rely on a hardcoded model selection — Ergouzi Switch fetches a **live model list** from the ChatGPT backend on demand.
...
Switching a Claude Desktop provider writes Ergouzi Switch's managed 3P profile and **requires restarting Claude Desktop** to take effect; proxy-mode providers require Ergouzi Switch's Local Routing to stay running while in use.
...
### Ergouzi Switch Brand Surface Unified to ergouzi.life

All in-app and README "official website" references now point at ergouzi.life as the sole official site; the release notes template also surfaces ergouzi.life.
...
The release notes template now surfaces `ergouzi.life`.

### Brand Surface

Documented `ergouzi.life` as the sole official website across READMEs and in-app references.
...
**Claude Desktop Third-Party Provider Switching via Proxy Gateway**: Routing Claude Desktop traffic through Ergouzi Switch's in-app proxy gateway to a third-party provider exposes those requests to that provider's billing, compliance, and data-retention policies — read the target provider's terms of service before using.

By enabling these features, users **accept all associated risks**. Ergouzi Switch is not responsible for any account restrictions, warnings, or service suspensions that result from using these features.

---

## Download & Installation

Visit [Releases](https://github.com/suboxiu88-coder/ergouzi-switch/releases/latest) to download the appropriate version.
...
| File                                     | Description                                           |
| ---------------------------------------- | ----------------------------------------------------- |
| `Ergouzi-Switch-v3.15.0-Windows.msi`          | **Recommended** - MSI installer, supports auto-update |
| `Ergouzi-Switch-v3.15.0-Windows-Portable.zip` | Portable, extract and run, no registry writes         |

### macOS

| File                             | Description                                             |
| -------------------------------- | ------------------------------------------------------- |
| `Ergouzi-Switch-v3.15.0-macOS.dmg`    | **Recommended** - DMG installer, drag into Applications |
| `Ergouzi-Switch-v3.15.0-macOS.zip`    | Extract and drag into Applications, Universal Binary    |
| `Ergouzi-Switch-v3.15.0-macOS.tar.gz` | For Homebrew installation and auto-update               |
...
>
> - `Ergouzi-Switch-v3.15.0-Linux-x86_64.AppImage` / `.deb` / `.rpm`
> - `Ergouzi-Switch-v3.15.0-Linux-arm64.AppImage` / `.deb` / `.rpm`

| Distribution                            | Recommended | Installation                                                           |
| --------------------------------------- | ----------- | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i Ergouzi-Switch-*.deb` or `sudo apt install ./Ergouzi-Switch-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i Ergouzi-Switch-*.rpm` or `sudo dnf install ./Ergouzi-Switch-*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./Ergouzi-Switch-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage` | Add execute permission and run, or use AUR                             |
| Other distros / not sure                | `.AppImage` | `chmod +x Ergouzi-Switch-*.AppImage && ./Ergouzi-Switch-*.AppImage`              |
