...
The Claude Desktop panel lets you manage Claude Desktop provider configurations in Ergouzi Switch.
...
Claude Desktop and Claude Code are separate app entry points. Claude Code uses `~/.claude/settings.json`, while Claude Desktop uses its own 3P profile configuration. In Ergouzi Switch they appear as separate apps: "Claude" and "Claude Desktop"; the icon badge in the bottom-right corner helps distinguish them.
...
| Third-party mode | Writes the 3P profile managed by Ergouzi Switch |
| MCP / Skills | Claude Desktop 3P profiles do not use Ergouzi Switch MCP / Skills sync |
...
- Providers that need routing: keep Ergouzi Switch running, turn on Claude Desktop local routing, then restart Claude Desktop
...
In direct mode, Ergouzi Switch points the Claude Desktop 3P profile to the provider endpoint:
...
- Ergouzi Switch local routing does not need to stay running while in use
...
Model mapping mode is for providers that expose non-Claude models, such as DeepSeek and Kimi, or when Ergouzi Switch needs to convert the API format.

After **Needs model mapping** is enabled, Claude Desktop connects to the Ergouzi Switch local gateway:

```text
http://127.0.0.1:15721/claude-desktop
```

Ergouzi Switch handles:
...
4. Using the provider credentials saved in Ergouzi Switch to call upstream
...
The reason is that Claude Desktop now rejects non-`claude-*` models, so Ergouzi Switch local routing performs a model mapping pass.
...
Model mapping mode needs Ergouzi Switch local routing to convert requests. Local routing is powerful but more complex, so the main-page routing toggle is hidden by default to avoid accidental clicks by users who do not need routing. When you need routing, show it manually.
...
Ergouzi Switch restores Claude Desktop's official 1P mode and removes the 3P profile managed by Ergouzi Switch.

Official mode does not need an API key or local routing.

When importing from Claude Code, Ergouzi Switch automatically adds **Claude Desktop Official**.

## Configuration File Locations

Ergouzi Switch writes Claude Desktop 3P configuration files.
...
These files are maintained automatically by Ergouzi Switch. Manual edits are not recommended. If configuration becomes inconsistent, enabling the current provider again usually repairs it.
...
| Local routing token has not been generated | Switch to the provider again; Ergouzi Switch will write a new local token |
...
- Ergouzi Switch is still running
...
- Model mapping mode: Ergouzi Switch must keep running, and Claude Desktop local routing must stay on

### Are real upstream model names written into Claude Desktop?

Not in model mapping mode. The Claude Desktop profile only stores safe `claude-*` routes and display names. Real upstream model names stay in the Ergouzi Switch provider configuration and are mapped when requests pass through the local gateway.
