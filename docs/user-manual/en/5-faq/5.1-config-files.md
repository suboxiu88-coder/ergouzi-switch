...
## Ergouzi Switch Data Storage

### Storage Directory

Default location: `~/.ergouzi-switch/`
...
```
~/.ergouzi-switch/
├── ergouzi-switch.db      # SQLite database (SSOT)
...
`ergouzi-switch.db` is a SQLite database that stores:
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to Ergouzi Switch's "providers") |
...
Ergouzi Switch's priority when modifying configurations:

1. **Ergouzi Switch Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by Ergouzi Switch)
- Ergouzi Switch's `settings.json`

### Not Recommended to Edit Manually

- `ergouzi-switch.db` database file
- Backup files
...
Ergouzi Switch v3.7.0 migrated from JSON files to SQLite:
