...
Ergouzi Switch for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/suboxiu88-coder/ergouzi-switch/releases).
...
2. Add Ergouzi Switch to your antivirus software's whitelist
...
chmod +x Ergouzi-Switch-*.AppImage

# If it still fails, try
./Ergouzi-Switch-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Check if the `~/.ergouzi-switch/` directory exists
2. Restore from backup: `~/.ergouzi-switch/backups/`
...
**Solutions**:
1. Confirm the file was exported by Ergouzi Switch
...
**Windows**:
- Check taskbar settings to ensure the Ergouzi Switch icon is not hidden
...
2. Restart the app
3. Delete `~/.ergouzi-switch/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask ergouzi-switch`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and Ergouzi Switch runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, Ergouzi Switch covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `ergouziswitch://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/suboxiu88-coder/ergouzi-switch/issues)
2. Search for similar issues
3. If none found, create a new Issue
4. Provide the following information:
   - Operating system and version
   - Ergouzi Switch version
...
- macOS/Linux: `~/.ergouzi-switch/logs/`
- Windows: `%APPDATA%\ergouzi-switch\logs\`
