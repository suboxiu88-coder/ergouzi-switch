...
This section describes how to configure Ergouzi Switch according to your preferences.
...
Ergouzi Switch supports three languages:
...
When enabled, Ergouzi Switch automatically runs when the system starts.
...
Starting from v3.13.0, Ergouzi Switch adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger Ergouzi Switch via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If Ergouzi Switch runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, Ergouzi Switch automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that Ergouzi Switch uses when opening a terminal.
...
The storage location for Ergouzi Switch's own data, defaulting to `~/.ergouzi-switch/`.
...
Configure Ergouzi Switch's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, Ergouzi Switch automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `ergouzi-switch-sync`) |
...
- Ergouzi Switch automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, Ergouzi Switch displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current Ergouzi Switch version number, with support for:
