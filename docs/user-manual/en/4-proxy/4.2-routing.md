...
App routing means letting Ergouzi Switch route a specific application's API requests through the local routing service.
...
When routing is enabled, Ergouzi Switch modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, Ergouzi Switch will:
...
Before enabling routing, Ergouzi Switch backs up the original configuration and restores it when disabled.
