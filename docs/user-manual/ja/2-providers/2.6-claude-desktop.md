...
Claude Desktop パネルでは、Ergouzi Switch 上で Claude Desktop のプロバイダー設定を管理できます。
...
Claude Desktop と Claude Code は別々のアプリ入口です。Claude Code は `~/.claude/settings.json` を使い、Claude Desktop は専用の 3P profile 設定を使います。Ergouzi Switch 上でも「Claude」と「Claude Desktop」として別々に表示され、アイコン右下の小さなバッジで区別できます。
...
| サードパーティモード | Ergouzi Switch が管理する 3P profile に書き込み |
| MCP / Skills | Claude Desktop 3P profile は Ergouzi Switch の MCP / Skills 同期を使いません |
...
多くのユーザーはまず Claude Code 側でプロバイダーを設定し、その同じプロバイダー群を Claude Desktop に持ち込みたいはずです。Ergouzi Switch の初回起動時、または Claude Desktop パネルを初めて開いたときにプロバイダーがまだない場合は、**Claude Code の既存プロバイダーをインポート** をクリックします。
...
- ルーティングが必要なプロバイダー：Ergouzi Switch を起動したまま、Claude Desktop ローカルルーティングをオンにし、Claude Desktop を再起動します
...
直結モードでは、Ergouzi Switch が Claude Desktop の 3P profile をプロバイダーのエンドポイントへ向けます。
...
- 形式変換が不要
- 使用中に Ergouzi Switch のローカルルーティングを起動し続ける必要がない
...
モデルマッピングモードは、DeepSeek や Kimi など Claude 系列ではないモデルを提供するプロバイダー、または Ergouzi Switch による API 形式変換が必要な場合に適しています。

**モデルマッピングが必要** をオンにすると、Claude Desktop は Ergouzi Switch のローカルゲートウェイへ接続します。

```text
http://127.0.0.1:15721/claude-desktop
```

Ergouzi Switch は次を担当します。
...
4. Ergouzi Switch に保存されたプロバイダー認証情報でアップストリームへアクセスする
...
理由は、現在の Claude Desktop が非 `claude-*` モデルを拒否するためです。そのため Ergouzi Switch のルーティング機能で一度モデルマッピングを行います。
...
モデルマッピングモードでは、リクエスト変換のために Ergouzi Switch ローカルルーティングが必要です。ローカルルーティングは強力ですが少し複雑な機能でもあるため、ルーティングを必要としないユーザーの誤操作を避けるために、メインページのルーティング切り替えはデフォルトで非表示です。必要なときだけ手動で表示してください。
...
Ergouzi Switch は Claude Desktop の公式 1P モードを復元し、Ergouzi Switch が管理する 3P profile を削除します。

公式モードでは API Key もローカルルーティングも不要です。

Claude Code からプロバイダーをインポートするとき、Ergouzi Switch は自動的に **Claude Desktop Official** も追加します。

## 設定ファイルの場所

Ergouzi Switch は Claude Desktop の 3P 設定ディレクトリに書き込みます。
...
これらのファイルは Ergouzi Switch が自動管理します。手動編集はおすすめしません。設定の不整合が起きた場合は、現在のプロバイダーを再度有効化すると通常は修復できます。
...
| ローカルルーティング token が未生成 | そのプロバイダーへ再度切り替えると、Ergouzi Switch が新しい token を書き込みます |
...
### Ergouzi Switch を閉じてもよい？

モードによります。

- 直結モード：Claude Desktop が再起動して設定を読み込んだ後は、ローカルルーティングを起動し続ける必要はありません
- モデルマッピングモード：Ergouzi Switch を起動したままにし、Claude Desktop ローカルルーティングもオンにしておく必要があります

### 実際のアップストリームモデル名は Claude Desktop に書き込まれる？

モデルマッピングモードでは書き込まれません。Claude Desktop profile には安全な `claude-*` ルートと表示名だけが保存されます。実際のアップストリームモデル名は Ergouzi Switch のプロバイダー設定に保存され、ローカルゲートウェイを通るリクエスト時にマッピングされます。
