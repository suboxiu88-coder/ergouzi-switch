...
## Ergouzi Switch のデータストレージ

### ストレージディレクトリ

デフォルトの場所：`~/.ergouzi-switch/`
...
```
~/.ergouzi-switch/
├── ergouzi-switch.db      # SQLite データベース（SSOT）
...
`ergouzi-switch.db` は SQLite データベースで、以下を保存しています：
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（Ergouzi Switch の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
Ergouzi Switch が設定を変更する際の優先順位：

1. **Ergouzi Switch データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（Ergouzi Switch がバックフィルする）
- Ergouzi Switch の `settings.json`

### 手動編集を推奨しないもの

- `ergouzi-switch.db` データベースファイル
- バックアップファイル
...
Ergouzi Switch v3.7.0 で JSON ファイルから SQLite に移行しました：
