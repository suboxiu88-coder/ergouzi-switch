...
```
~/.ergouzi-switch/
├── ergouzi-switch.db      # SQLite 数据库（SSOT）
├── settings.json     # 设备级设置
├── skills/           # 技能 SSOT 目录
...
`ergouzi-switch.db` 是 SQLite 数据库，存储：
...
| 字段 | 说明 |
|------|------|
| `models.providers` | 供应商配置（映射为 Ergouzi Switch 的"供应商"） |
| `env` | 环境变量配置 |
...
Ergouzi Switch 修改配置时的优先级：

1. **Ergouzi Switch 数据库** - 单一事实源 (SSOT)
...
- CLI 工具的配置文件（会被 Ergouzi Switch 回填）
- Ergouzi Switch 的 `settings.json`

### 不建议手动编辑

- `ergouzi-switch.db` 数据库文件
- 备份文件
...
Ergouzi Switch v3.7.0 从 JSON 文件迁移到 SQLite：
